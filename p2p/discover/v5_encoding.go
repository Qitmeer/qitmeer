...
type authResponse struct {
	Version   uint
	Signature []byte
	Record    *qnr.Record `rlp:"nil"` // sender's record
}
...
func newWireCodec(ln *qnode.LocalNode, key *ecdsa.PrivateKey, clock mclock.Clock) *wireCodec {
	c := &wireCodec{
		sha256:    sha256.New(),
		localnode: ln,
		privkey:   key,
...
// node.
func (c *wireCodec) encode(id qnode.ID, addr string, packet packetV5, challenge *whoareyouV5) ([]byte, []byte, error) {
...
func (c *wireCodec) encodeRandom(toID qnode.ID) ([]byte, []byte, error) {
...
func (c *wireCodec) encodeWhoareyou(toID qnode.ID, packet *whoareyouV5) ([]byte, error) {
...
func (c *wireCodec) encodeEncrypted(toID qnode.ID, toAddr string, packet packetV5, writeKey []byte, challenge *whoareyouV5) (enc []byte, authTag []byte, err error) {
	nonce := make([]byte, gcmNonceSize)
	if _, err := crand.Read(nonce); err != nil {
		return nil, nil, fmt.Errorf("can't get random data: %v", err)
	}
...
func (c *wireCodec) deriveKeys(n1, n2 qnode.ID, priv *ecdsa.PrivateKey, pub *ecdsa.PublicKey, challenge *whoareyouV5) *handshakeSecrets {
	eph := ecdh(priv, pub)
	if eph == nil {
		return nil
	}
...
func (c *wireCodec) decode(input []byte, addr string) (qnode.ID, *qnode.Node, packetV5, error) {
...
func (c *wireCodec) decodeEncrypted(fromID qnode.ID, fromAddr string, input []byte) (packetV5, *qnode.Node, error) {
...
func (c *wireCodec) decodeAuth(fromID qnode.ID, fromAddr string, head *authHeader) ([]byte, *qnode.Node, error) {
	if !head.isHandshake {
...
func (c *wireCodec) decodeAuthResp(fromID qnode.ID, fromAddr string, head *authHeaderList, challenge *whoareyouV5) (*handshakeSecrets, *qnode.Node, error) {
	...
	if resp.Record != nil {
		if node == nil || node.Seq() < resp.Record.Seq() {
			n, err := qnode.New(qnode.ValidSchemes, resp.Record)
			if err != nil {
...
func (c *wireCodec) verifyIDSignature(nonce, ephkey, sig []byte, n *qnode.Node) error {
...
func xorTag(a []byte, b qnode.ID) qnode.ID {
	var r qnode.ID
	for i := range r {
		r[i] = a[i] ^ b[i]
	}
