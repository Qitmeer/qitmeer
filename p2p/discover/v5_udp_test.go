...
func (test *udpV5Test) expectNodes(wantReqID []byte, wantTotal uint8, wantNodes []*qnode.Node) {
	nodeSet := make(map[qnode.ID]*qnr.Record)
...
func (c *testCodec) encode(toID qnode.ID, addr string, p packetV5, _ *whoareyouV5) ([]byte, []byte, error) {
	c.ctr++
	authTag := make([]byte, 8)
...
func (c *testCodec) decode(input []byte, addr string) (qnode.ID, *qnode.Node, packetV5, error) {
	frame, p, err := c.decodeFrame(input)
	if err != nil {
		return qnode.ID{}, nil, nil, err
	}
	if p.kind() == p_whoareyouV5 {
		frame.NodeID = qnode.ID{} // match wireCodec behavior
	}
