...
func newTable(t transport, db *qnode.DB, bootnodes []*qnode.Node) (*Table, error) {
	tab := &Table{
		net:        t,
		db:         db,
...
func (tab *Table) doRevalidate(done chan<- struct{}) {
	...
	if last.Seq() < remoteSeq {
		...
		if err != nil {
			log.Debug("QNR request failed", "id", last.ID(), "addr", last.addr(), "err", err)
		} else {
...
func (tab *Table) closest(target qnode.ID, nresults int, checklive bool) *nodesByDistance {
