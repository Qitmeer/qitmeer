...
type codecV5 interface {
	...
	// WHOAREYOU response.
	encode(fromID qnode.ID, fromAddr string, p packetV5, challenge *whoareyouV5) (enc []byte, authTag []byte, err error)
	...
	decode(input []byte, fromAddr string) (fromID qnode.ID, fromNode *qnode.Node, p packetV5, err error)
}
...
type packetV5 interface {
	...
	handle(t *UDPv5, fromID qnode.ID, fromAddr *net.UDPAddr)
}
...
func newUDPv5(conn UDPConn, ln *qnode.LocalNode, cfg Config) (*UDPv5, error) {
	...
	tab, err := newTable(t, t.db, cfg.Bootnodes)
	if err != nil {
		return nil, err
	}
	t.tab = tab
...
func lookupDistances(target, dest qnode.ID) (dists []int) {
...
func (t *UDPv5) ping(n *qnode.Node) (uint64, error) {
	resp := t.call(n, p_pongV5, &pingV5{QNRSeq: t.localNode.Node().Seq()})
	defer t.callDone(resp)
	select {
	case pong := <-resp.ch:
		return pong.(*pongV5).QNRSeq, nil
	case err := <-resp.err:
		return 0, err
	}
}
...
func (t *UDPv5) requestTicket(n *qnode.Node) ([]byte, error) {
...
func (t *UDPv5) findnode(n *qnode.Node, distance int) ([]*qnode.Node, error) {
...
func (t *UDPv5) waitForNodes(c *callV5, distance int) ([]*qnode.Node, error) {
	defer t.callDone(c)
...
func (t *UDPv5) verifyResponseNode(c *callV5, r *qnr.Record, distance int, seen map[qnode.ID]struct{}) (*qnode.Node, error) {
	...
	if distance != -1 {
		if d := qnode.LogDist(c.node.ID(), node.ID()); d != distance {
...
func (t *UDPv5) call(node *qnode.Node, responseType byte, packet packetV5) *callV5 {
	c := &callV5{
		node:         node,
		packet:       packet,
...
func (t *UDPv5) sendResponse(toID qnode.ID, toAddr *net.UDPAddr, packet packetV5) error {
	_, err := t.send(toID, toAddr, packet, nil)
	return err
}

// send sends a packet to the given node.
func (t *UDPv5) send(toID qnode.ID, toAddr *net.UDPAddr, packet packetV5, c *whoareyouV5) ([]byte, error) {
	addr := toAddr.String()
	enc, authTag, err := t.codec.encode(toID, addr, packet, c)
	if err != nil {
		log.Warn(">> "+packet.name(), "id", toID, "addr", addr, "err", err)
		return authTag, err
	}
	_, err = t.conn.WriteToUDP(enc, toAddr)
	log.Trace(">> "+packet.name(), "id", toID, "addr", addr)
	return authTag, err
}
...
func (t *UDPv5) handlePacket(rawpacket []byte, fromAddr *net.UDPAddr) error {
	...
	if packet.kind() != p_whoareyouV5 {
		...
		log.Trace("<< "+packet.name(), "id", fromID, "addr", addr)
	}
...
func (t *UDPv5) handleCallResponse(fromID qnode.ID, fromAddr *net.UDPAddr, reqid []byte, p packetV5) {
	...
	if p.kind() != ac.responseType {
		log.Debug(fmt.Sprintf("Wrong disv5 response type %s", p.name()), "id", fromID, "addr", fromAddr)
		return
	}
...
func (p *unknownV5) handle(t *UDPv5, fromID qnode.ID, fromAddr *net.UDPAddr) {
...
func (p *whoareyouV5) handle(t *UDPv5, fromID qnode.ID, fromAddr *net.UDPAddr) {
	c, err := p.matchWithCall(t, p.AuthTag)
	if err != nil {
		log.Debug("Invalid WHOAREYOU/v5", "addr", fromAddr, "err", err)
		return
	}
...
func (p *pingV5) handle(t *UDPv5, fromID qnode.ID, fromAddr *net.UDPAddr) {
...
func (p *pongV5) handle(t *UDPv5, fromID qnode.ID, fromAddr *net.UDPAddr) {
...
func (p *findnodeV5) handle(t *UDPv5, fromID qnode.ID, fromAddr *net.UDPAddr) {
	if p.Distance == 0 {
		t.sendNodes(fromID, fromAddr, p.ReqID, []*qnode.Node{t.Self()})
		return
	}
	if p.Distance > 256 {
		p.Distance = 256
...
func (t *UDPv5) sendNodes(toID qnode.ID, toAddr *net.UDPAddr, reqid []byte, nodes []*qnode.Node) {
	// TODO livenessChecks > 1
	// TODO CheckRelayIP
	total := uint8(math.Ceil(float64(len(nodes)) / 3))
	resp := &nodesV5{ReqID: reqid, Total: total, Nodes: make([]*qnr.Record, 3)}
	sent := false
	for len(nodes) > 0 {
...
func (p *nodesV5) handle(t *UDPv5, fromID qnode.ID, fromAddr *net.UDPAddr) {
	t.handleCallResponse(fromID, fromAddr, p.ReqID, p)
}
...
func (p *requestTicketV5) handle(t *UDPv5, fromID qnode.ID, fromAddr *net.UDPAddr) {
...
func (p *ticketV5) handle(t *UDPv5, fromID qnode.ID, fromAddr *net.UDPAddr) {
	t.handleCallResponse(fromID, fromAddr, p.ReqID, p)
}
...
func (p *regtopicV5) handle(t *UDPv5, fromID qnode.ID, fromAddr *net.UDPAddr) {
...
func (p *regconfirmationV5) handle(t *UDPv5, fromID qnode.ID, fromAddr *net.UDPAddr) {
	t.handleCallResponse(fromID, fromAddr, p.ReqID, p)
}
...
func (p *topicqueryV5) handle(t *UDPv5, fromID qnode.ID, fromAddr *net.UDPAddr) {
}
