...
func (tn *preminedTestnet) neighborsAtDistance(base *qnode.Node, distance uint, elems int) []*qnode.Node {
	nodes := nodesByDistance{target: base.ID()}
	for d := range lookupTestnet.dists {
		for i := range lookupTestnet.dists[d] {
			n := lookupTestnet.node(d, i)
			if uint(qnode.LogDist(n.ID(), base.ID())) == distance {
...
func (tn *preminedTestnet) closest(n int) (nodes []*qnode.Node) {
	...
	sort.Slice(nodes, func(i, j int) bool {
		return qnode.DistCmp(tn.target.id(), nodes[i].ID(), nodes[j].ID()) < 0
	})
	return nodes[:n]
}
