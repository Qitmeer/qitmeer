...
import (
	...
	chaincfg "github.com/HalalChain/qitmeer-lib/params/dcr"
...
func encodePKAddress(serializedPK []byte, netID [2]byte, algo chainec.EcType) string {
	pubKeyBytes := []byte{0x00}

	switch algo {
	case chainec.ECDSA_Secp256k1:
		pubKeyBytes[0] = byte(chainec.ECDSA_Secp256k1)
	case chainec.EdDSA_Ed25519:
		pubKeyBytes[0] = byte(chainec.EdDSA_Ed25519)
	case chainec.ECDSA_SecpSchnorr:
		pubKeyBytes[0] = byte(chainec.ECDSA_SecpSchnorr)
	}

	// Pubkeys are encoded as [0] = type/ybit, [1:33] = serialized pubkey
	compressed := serializedPK
	if algo == chainec.ECDSA_Secp256k1 || algo == chainec.ECDSA_SecpSchnorr {
...
type Address interface {
	...
	DSA(*chaincfg.Params) chainec.EcType
...
func DecodeAddress(addr string) (Address, error) {
	...
	case net.PubKeyHashAddrID:
		return NewAddressPubKeyHash(decoded, net, chainec.ECDSA_Secp256k1)

	case net.PKHEdwardsAddrID:
		return NewAddressPubKeyHash(decoded, net, chainec.EdDSA_Ed25519)

	case net.PKHSchnorrAddrID:
		return NewAddressPubKeyHash(decoded, net, chainec.ECDSA_SecpSchnorr)
...
func NewAddressPubKeyHash(pkHash []byte, net *chaincfg.Params,
	algo chainec.EcType) (*AddressPubKeyHash, error) {
	var addrID [2]byte
	switch algo {
	case chainec.ECDSA_Secp256k1:
...
// hash.
func (a *AddressPubKeyHash) DSA(net *chaincfg.Params) chainec.EcType {
	switch a.netID {
	case net.PubKeyHashAddrID:
		return chainec.ECDSA_Secp256k1
	case net.PKHEdwardsAddrID:
		return chainec.EdDSA_Ed25519
	case net.PKHSchnorrAddrID:
		return chainec.ECDSA_SecpSchnorr
	}
	return -1
}
...
func NewAddressScriptHash(serializedScript []byte,
	...
	scriptHash := dcr.Hash160(serializedScript)
...
func (a *AddressScriptHash) DSA(net *chaincfg.Params) chainec.EcType {
	return -1
}
...
func (a *AddressSecpPubKey) EncodeAddress() string {
	return encodeAddress(dcr.Hash160(a.serialize()), a.pubKeyHashID)
}
...
func (a *AddressSecpPubKey) Hash160() *[ripemd160.Size]byte {
	h160 := dcr.Hash160(a.pubKey.SerializeCompressed())
...
func (a *AddressSecpPubKey) AddressPubKeyHash() *AddressPubKeyHash {
	...
	copy(addr.hash[:], dcr.Hash160(a.serialize()))
	return addr
}
...
// address.
func (a *AddressSecpPubKey) DSA(net *chaincfg.Params) chainec.EcType {
...
func NewAddressEdwardsPubKey(serializedPubKey []byte,
	...
	pubKey, err := chainec.Ed25519.ParsePubKey(serializedPubKey)
	if err != nil {
		return nil, err
	}
...
func (a *AddressEdwardsPubKey) Hash160() *[ripemd160.Size]byte {
	h160 := dcr.Hash160(a.pubKey.Serialize())
	array := new([ripemd160.Size]byte)
	copy(array[:], h160)
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressEdwardsPubKey) DSA(net *chaincfg.Params) chainec.EcType {
	return chainec.EdDSA_Ed25519
}
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressSecSchnorrPubKey) DSA(net *chaincfg.Params) chainec.EcType {
	return chainec.ECDSA_SecpSchnorr
}
