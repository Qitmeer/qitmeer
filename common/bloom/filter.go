...
type Filter struct {
	mtx           sync.Mutex
	msgFilterLoad *types.MsgFilterLoad
}
...
func NewFilter(elements, tweak uint32, fprate float64, flags types.BloomUpdateType) *Filter {
	...
	dataLen = minUint32(dataLen, types.MaxFilterLoadFilterSize*8) / 8
	...
	hashFuncs = minUint32(hashFuncs, types.MaxFilterLoadHashFuncs)

	data := make([]byte, dataLen)
	msg := types.NewMsgFilterLoad(data, hashFuncs, tweak, flags)
...
// types.MsgFilterLoad.
func LoadFilter(filter *types.MsgFilterLoad) *Filter {
	return &Filter{
...
//
// This function is safe for concurrent access.
func (bf *Filter) Reload(filter *types.MsgFilterLoad) {
	bf.mtx.Lock()
...
//
// This function MUST be called with the filter lock held.
func (bf *Filter) matchesOutPoint(outpoint *types.TxOutPoint) bool {
	// Serialize
	var buf [chainhash.HashSize + 4]byte
	copy(buf[:], outpoint.Hash[:])
	binary.LittleEndian.PutUint32(buf[chainhash.HashSize:], outpoint.OutIndex)
...
//
// This function MUST be called with the filter lock held.
func (bf *Filter) addOutPoint(outpoint *types.TxOutPoint) {
	// Serialize
	var buf [chainhash.HashSize + 4]byte
	copy(buf[:], outpoint.Hash[:])
	binary.LittleEndian.PutUint32(buf[chainhash.HashSize:], outpoint.OutIndex)
...
func (bf *Filter) maybeAddOutpoint(pkScript []byte, outHash *chainhash.Hash, outIdx uint32) {
	...
	case types.BloomUpdateAll:
		outpoint := types.NewOutPoint(outHash, outIdx)
		bf.addOutPoint(outpoint)
...
//
// This function MUST be called with the filter lock held.
func (bf *Filter) matchTxAndUpdate(tx *types.Tx) bool {
	...
	for _, txin := range tx.Tx.TxIn {
		if bf.matchesOutPoint(&txin.PreviousOut) {
			return true
		}

		pushedData, err := txscript.PushedData(txin.SignScript)
		if err != nil {
			continue
		}
		for _, data := range pushedData {
...
//
// This function is safe for concurrent access.
func (bf *Filter) MsgFilterLoad() *types.MsgFilterLoad {
	bf.mtx.Lock()
