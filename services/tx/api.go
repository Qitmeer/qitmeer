...
func (api *PublicTxAPI) GetRawTransaction(txHash hash.Hash, verbose bool) (interface{}, error) {
	...
	tx, inRecentBlock, _ := api.txManager.txMemPool.FetchTransaction(&txHash, true)
	...
	if inRecentBlock {
		blkOrder = api.txManager.bm.GetChain().BestSnapshot().Order
		confirmations = 1
	} else if tx != nil {
		confirmations = 0
	} else {
...
func (api *PublicTxAPI) GetUtxo(txHash hash.Hash, vout uint32, includeMempool *bool) (interface{}, error) {
	...
	if includeMempoolTx {
		txFromMempool, inRecentBlock, _ := api.txManager.txMemPool.FetchTransaction(&txHash, true)
