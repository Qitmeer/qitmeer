...
func (m *CPUMiner) solveCuckarooBlock(msgBlock *types.Block, ticker *time.Ticker, quit chan struct{}, scale ,height uint64) bool {
	...
	for i := uint32(0); i <= maxNonce; i++ {
		...
		if pow.CalcCuckooDiff(pow.GraphWeight(uint32(cuckoo.Edgebits),int64(height),m.params.PowConfig.AdjustmentStartMainHeight,pow.CUCKAROO), header.BlockHash()).Cmp(targetDiff) >= 0 {
			m.updateHashes <- hashesCompleted
			return true
		}
	}
