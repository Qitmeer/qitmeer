...
import (
	...
	hashers "github.com/dindinw/dagproject/merkletree/hasher"
...
func NewCompactMerkleTreeWithState(hasher hashers.LogHasher, size int64, f GetNodeFunc, expectedRoot []byte) (*CompactMerkleTree, error) {
	...
	if isPerfectTree(size) {
		...
		for depth := 0; depth < sizeBits; depth++ {
			if size&1 == 1 {
				index := size - 1
				log.Info(fmt.Sprintf("fetching d: %d i: %d, leaving size %d", depth, index, size))
				h, err := f(depth, index)
				if err != nil {
					log.Warn(fmt.Sprintf("Failed to fetch node depth %d index %d: %s", depth, index, err))
					return nil, err
				}
				r.nodes[depth] = h
			}
	...
	if !bytes.Equal(r.root, expectedRoot) {
		log.Warn(fmt.Sprintf("Corrupt state, expected root %s, got %s", hex.EncodeToString(expectedRoot[:]), hex.EncodeToString(r.root[:])))
		return nil, RootHashMismatchError{ActualHash: r.root, ExpectedHash: expectedRoot}
	}
	log.Info(fmt.Sprintf("Resuming at size %d, with root: %s", r.size, base64.StdEncoding.EncodeToString(r.root[:])))
	return &r, nil
}
...
func (c CompactMerkleTree) DumpNodes() {
	log.Info(fmt.Sprintf("Tree Nodes @ %d", c.size))
	mask := int64(1)
	numBits := bitLen(c.size)
	for bit := 0; bit < numBits; bit++ {
		if c.size&mask != 0 {
			log.Info(fmt.Sprintf("%d:  %s", bit, base64.StdEncoding.EncodeToString(c.nodes[bit][:])))
		} else {
			log.Info(fmt.Sprintf("%d:  -", bit))
		}
		mask <<= 1
	}
}
