...
var (
	logHashers = make(map[HashStrategy]LogHasher)
	mapHashers = make(map[HashStrategy]MapHasher)
)

// RegisterLogHasher registers a hasher for use.
func RegisterLogHasher(h HashStrategy, f LogHasher) {
	if h == HashStrategy_UNKNOWN_HASH_STRATEGY {
		panic(fmt.Sprintf("RegisterLogHasher(%d) of unknown hasher", h))
	}
	if logHashers[h] != nil {
...
func RegisterMapHasher(h HashStrategy, f MapHasher) {
	if h == HashStrategy_UNKNOWN_HASH_STRATEGY {
		panic(fmt.Sprintf("RegisterMapHasher(%d) of unknown hasher", h))
	}
	if mapHashers[h] != nil {
...
func NewLogHasher(h HashStrategy) (LogHasher, error) {
	f := logHashers[h]
	if f != nil {
		return f, nil
	}
	return nil, fmt.Errorf("LogHasher(%d) is an unknown hasher", h)
}

// NewMapHasher returns a MapHasher.
func NewMapHasher(h HashStrategy) (MapHasher, error) {
	f := mapHashers[h]
	if f != nil {
		return f, nil
	}
	return nil, fmt.Errorf("MapHasher(%d) is an unknown hasher", h)
}
