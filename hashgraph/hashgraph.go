...
import (
    ...
    "github.com/dindinw/dagproject/log"
	"os"
)
...
type Hashgraph struct {
	...
	ancestorCache           *LRU
	selfAncestorCache       *LRU
	oldestSelfAncestorCache *LRU
	stronglySeeCache        *LRU
	parentRoundCache        *LRU
	roundCache              *LRU
...
func NewHashgraph(participants map[string]int, store Store, commitCh chan []Event) Hashgraph {
    logger := log.New("hashgraph","hashgraph-log")
	...
	return Hashgraph{
		...
		Store:                   store,
		commitCh:                commitCh,
		ancestorCache:           NewLRU(cacheSize, nil),
		selfAncestorCache:       NewLRU(cacheSize, nil),
		oldestSelfAncestorCache: NewLRU(cacheSize, nil),
		stronglySeeCache:        NewLRU(cacheSize, nil),
		parentRoundCache:        NewLRU(cacheSize, nil),
...
func (h *Hashgraph) ancestor(x, y string) bool {
	...
	//fmt.Fprintf(os.Stdout,"ancestor DEBUG: x=%v, y=%v \n",x,y)
	//fmt.Fprintf(os.Stdout,"ancestor DEBUG: event_x=%v \n",ex);
	//fmt.Fprintf(os.Stdout,"ancestor DEBUG: event_y=%v \n",ey);

	eyCreator := h.Participants[ey.Creator()]

	//fmt.Fprintf(os.Stdout,"ancestor DEBUG: h.Participants=%v \n",h.Participants)
	//fmt.Fprintf(os.Stdout,"ancestor DEBUG: eyCreator=%v \n",eyCreator)
	//fmt.Fprintf(os.Stdout,"ancestor DEBUG: ex.lastAncestors=%v \n",ex.lastAncestors)
	//fmt.Fprintf(os.Stdout,"ancestor DEBUG: lastAncestorKnownFromYCreator=%v \n",ex.lastAncestors[eyCreator].index)
	//fmt.Fprintf(os.Stdout,"ancestor DEBUG: ey.Index()=%v \n",ey.Index())
...
func (h *Hashgraph) InitEventCoordinates(event *Event) error {
    ...
    // [ 0 : 0x041585DA8F9DFB7BF629B45953B676367B1A9074A7A3BC84CCDA23F26C9016D4D76FAE2D65F669159AC4B400D4DC1A5A7C45E9BD59E8D0CB6B2BC73CF60C26E4E1 ]
    // [ 1 : 0x04CBB01D01DC82D7E354F295773C486D5C151820147C8355D2FED4374D8F4B05B5F28930C353D0B841D4814D5EA97FF0A4BD0B0F09F29DB22CE4EFA603F505880D ]
    // [ 2 : 0x040E9E718BEC51E2E41B5664D8ED0B671FBB51B7B03B1D0DE37AAB33D394583BC32B4A4BAB498F65FAED10DA98F3287C625A538FE9AD0B631038047C7A02F81AEC ]
	//for hash,id := range h.Participants {
	//	fmt.Fprintf(os.Stdout,"InitEventCoordinates: h.Participants [ %v : %v ] \n", id, hash);
	//}

	// InitEventCoordinates: members = 3
	// fmt.Fprintf(os.Stdout,"InitEventCoordinates: members = %v \n",members);
    ...
    // InitEventCoordinates: event.firstDescendants  9223372036854775807 ->
    // InitEventCoordinates: event.firstDescendants  9223372036854775807 ->
	// InitEventCoordinates: event.firstDescendants  9223372036854775807 ->
	/*
	for _, e := range event.firstDescendants {
		fmt.Fprintf(os.Stdout,"InitEventCoordinates: event.firstDescendants  %v -> %v  \n", e.index,e.hash );
	}
	fmt.Fprintf(os.Stdout,"InitEventCoordinates: event.firstDescendants  %+v  \n", event.firstDescendants);
	*/
	...
	if event.SelfParent() == "" && event.OtherParent() == "" {
	   ...
	   //   InitEventCoordinates: event.lastAncestors  -1 ->
	   //	InitEventCoordinates: event.lastAncestors  -1 ->
	   //	InitEventCoordinates: event.lastAncestors  -1 ->
	   //for _, e := range event.lastAncestors {
	   //	fmt.Fprintf(os.Stdout,"InitEventCoordinates: event.lastAncestors  %v -> %v  \n", e.index,e.hash );
	   //}
