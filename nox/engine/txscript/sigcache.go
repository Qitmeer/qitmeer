...
func NewSigCache(maxEntries uint) *SigCache {
	return &SigCache{
		validSigs:  make(map[hash.Hash]sigCacheEntry, maxEntries),
...
func (s *SigCache) Exists(sigHash hash.Hash, sig chainec.Signature, pubKey chainec.PublicKey) bool {
	s.RLock()
...
func (s *SigCache) Add(sigHash hash.Hash, sig chainec.Signature, pubKey chainec.PublicKey) {
	s.Lock()
	defer s.Unlock()
