...
func TestAddresses(t *testing.T) {
	tests := []struct {
		...
		// Positive P2PKH tests.
		{
			...
			valid:   true,
			result: btc.TstAddressPubKeyHash(
				[ripemd160.Size]byte{
		...
		// Positive P2SH tests.
		{
			...
			valid:   true,
			result: btc.TstAddressScriptHash(
				[ripemd160.Size]byte{
		...
		},
		{
			...
			result: btc.TstAddressPubKey(
				...
				btc.PKFHybrid, btc.MainNetParams.PubKeyHashAddrID),
			f: func() (btc.Address, error) {
				serializedPubKey := []byte{
					0x06, 0x19, 0x2d, 0x74, 0xd0, 0xcb, 0x94, 0x34, 0x4c, 0x95,
					0x69, 0xc2, 0xe7, 0x79, 0x01, 0x57, 0x3d, 0x8d, 0x79, 0x03,
		...
		},
		{
			...
			result: btc.TstAddressPubKey(
				[]byte{
					...
					0x76, 0x83, 0x01, 0x60, 0xc6, 0xd2, 0xeb, 0x5e, 0x6a, 0x4c,
					0x44, 0xd3, 0x3f, 0x45, 0x3e},
				btc.PKFHybrid, btc.TestNet3Params.PubKeyHashAddrID),
			f: func() (btc.Address, error) {
				serializedPubKey := []byte{
					0x06, 0x19, 0x2d, 0x74, 0xd0, 0xcb, 0x94, 0x34, 0x4c, 0x95,
					0x69, 0xc2, 0xe7, 0x79, 0x01, 0x57, 0x3d, 0x8d, 0x79, 0x03,
		...
		// Segwit address tests.
		{
			...
			valid:   true,
			result: btc.TstAddressWitnessPubKeyHash(
				0,
				[20]byte{
		...
		},
		{
			...
			valid:   true,
			result: btc.TstAddressWitnessScriptHash(
				0,
				[32]byte{
	...
	for _, test := range tests {
		...
		decoded, err := btc.DecodeAddress(test.addr, test.net)
		...
		if err == nil {
			...
			case *btc.AddressPubKeyHash:
				saddr = btc.TstAddressSAddr(encoded)

			case *btc.AddressScriptHash:
				saddr = btc.TstAddressSAddr(encoded)

			case *btc.AddressPubKey:
				// Ignore the error here since the script
				// address is checked below.
				saddr, _ = hex.DecodeString(d.String())
			case *btc.AddressWitnessPubKeyHash:
				saddr = btc.TstAddressSegwitSAddr(encoded)
			case *btc.AddressWitnessScriptHash:
				saddr = btc.TstAddressSegwitSAddr(encoded)
			}
			...
			case *btc.AddressWitnessPubKeyHash:
				...
				expVer := test.result.(*btc.AddressWitnessPubKeyHash).WitnessVersion()
			...
			case *btc.AddressWitnessScriptHash:
				...
				expVer := test.result.(*btc.AddressWitnessScriptHash).WitnessVersion()
				if v := a.WitnessVersion(); v != expVer {
					t.Errorf("%v: witness versions do not match:\n%x != \n%x",
						test.name, expVer, v)
					return
