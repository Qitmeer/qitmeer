...
func (t *TxIn) SerializeSize() int {
	...
	return 40 + s.VarIntSerializeSize(uint64(len(t.SignatureScript))) +
...
// BtcTx represents a bitcoin tx message.
...
func (msg *BtcTx) AddTxIn(ti *TxIn) {
	msg.TxIn = append(msg.TxIn, ti)
}

// AddTxOut adds a transaction output to the message.
func (msg *BtcTx) AddTxOut(to *TxOut) {
	msg.TxOut = append(msg.TxOut, to)
}

// TxHash generates the Hash for the transaction.
func (msg *BtcTx) TxHash() chainhash.Hash {
	...
	return btchash.DoubleHashH(buf.Bytes())
}
...
func (msg *BtcTx) WitnessHash() chainhash.Hash {
	if msg.HasWitness() {
...
func (msg *BtcTx) BtcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	version, err := s.BinarySerializer.Uint32(r, littleEndian)
	if err != nil {
		return err
	}
	...
	if count == 0 && enc == WitnessEncoding {
		...
		if flag[0] != 0x01 {
			...
			return messageError("BtcTx.BtcDecode", str)
		}
	...
	if flag[0] != 0 && enc == WitnessEncoding {
		for _, txin := range msg.TxIn {
			...
			witCount, err := s.ReadVarInt(r, pver)
			if err != nil {
				returnScriptBuffers()
				return err
			}
	...
	msg.LockTime, err = s.BinarySerializer.Uint32(r, littleEndian)
	if err != nil {
		returnScriptBuffers()
		return err
	}
...
// deal with changes.
func (msg *BtcTx) Deserialize(r io.Reader) error {
...
// within inputs.
func (msg *BtcTx) DeserializeNoWitness(r io.Reader) error {
...
func (msg *BtcTx) BtcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	err := s.BinarySerializer.PutUint32(w, littleEndian, uint32(msg.Version))
	if err != nil {
		return err
	}

	// If the encoding version is set to WitnessEncoding, and the Flags
	// field for the BtcTx aren't 0x00, then this indicates the transaction
	...
	return s.BinarySerializer.PutUint32(w, littleEndian, msg.LockTime)
}

// HasWitness returns false if none of the inputs within the transaction
// contain witness data, true false otherwise.
func (msg *BtcTx) HasWitness() bool {
	for _, txIn := range msg.TxIn {
...
// difference and separating the two allows the API to be flexible enough to
// deal with changes.
func (msg *BtcTx) Serialize(w io.Writer) error {
	// At the current time, there is no difference between the wire encoding
	// at protocol version 0 and the stable long-term storage format.  As
	// a result, make use of BtcEncode.
	//
	// Passing a encoding type of WitnessEncoding to BtcEncode for BtcTx
...
func (msg *BtcTx) SerializeNoWitness(w io.Writer) error {
...
func (msg *BtcTx) baseSize() int {
	...
	n := 8 + s.VarIntSerializeSize(uint64(len(msg.TxIn))) +
		s.VarIntSerializeSize(uint64(len(msg.TxOut)))
...
// SerializeSize returns the number of bytes it would take to serialize the
// the transaction.
func (msg *BtcTx) SerializeSize() int {
	n := msg.baseSize()
...
func (msg *BtcTx) SerializeSizeStripped() int {
	return msg.baseSize()
}

// Command returns the protocol command string for the message.  This is part
// of the Message interface implementation.
func (msg *BtcTx) Command() string {
	return CmdTx
}

// MaxPayloadLength returns the maximum length the payload can be for the
// receiver.  This is part of the Message interface implementation.
func (msg *BtcTx) MaxPayloadLength(pver uint32) uint32 {
...
func (msg *BtcTx) PkScriptLocs() []int {
	...
	// input.
	n := 4 + s.VarIntSerializeSize(uint64(len(msg.TxIn))) +
		s.VarIntSerializeSize(uint64(numTxOut))
	...
	for i, txOut := range msg.TxOut {
		...
		// PkScript.
		n += 8 + s.VarIntSerializeSize(uint64(len(txOut.PkScript)))
		pkScriptLocs[i] = n
...
// NewBtcTx returns a new bitcoin tx message that conforms to the Message
// interface.  The return instance has a default version of TxVersion and there
// are no transaction inputs or outputs.  Also, the lock time is set to zero
// to indicate the transaction is valid immediately as opposed to some time in
// future.
func NewBtcTx(version int32) *BtcTx {
	return &BtcTx{
		Version: version,
...
func readOutPoint(r io.Reader, pver uint32, version int32, op *OutPoint) error {
	...
	op.Index, err = s.BinarySerializer.Uint32(r, littleEndian)
	return err
}
...
func writeOutPoint(w io.Writer, pver uint32, version int32, op *OutPoint) error {
	...
	return s.BinarySerializer.PutUint32(w, littleEndian, op.Index)
}
...
func readTxIn(r io.Reader, pver uint32, version int32, ti *TxIn) error {
	...
	return s.ReadElements(r, &ti.Sequence)
}
...
func writeTxIn(w io.Writer, pver uint32, version int32, ti *TxIn) error {
	...
	err = s.WriteVarBytes(w, pver, ti.SignatureScript)
	if err != nil {
		return err
	}
...
func WriteTxOut(w io.Writer, pver uint32, version int32, to *TxOut) error {
	err := s.BinarySerializer.PutUint64(w, littleEndian, uint64(to.Value))
	if err != nil {
		return err
	}
...
func writeTxWitness(w io.Writer, pver uint32, version int32, wit [][]byte) error {
	err := s.WriteVarInt(w, pver, uint64(len(wit)))
	if err != nil {
		return err
	}
	for _, item := range wit {
		err = s.WriteVarBytes(w, pver, item)
		if err != nil {
			return err
		}
	}
