...
func compressedScriptSize(pkScript []byte) int {
	...
	return serialization.SerializeSizeVLQ(uint64(len(pkScript)+numSpecialScripts)) +
		len(pkScript)
}
...
func decodeCompressedScriptSize(serialized []byte) int {
	scriptSize, bytesRead := serialization.DeserializeVLQ(serialized)
	if bytesRead == 0 {
		return 0
	}
...
func putCompressedScript(target []byte, pkScript []byte) int {
	...
	vlqSizeLen := serialization.PutVLQ(target, encodedSize)
...
func decompressScript(compressedPkScript []byte) []byte {
	...
	encodedScriptSize, bytesRead := serialization.DeserializeVLQ(compressedPkScript)
...
func compressedTxOutSize(amount uint64, pkScript []byte) int {
	return serialization.SerializeSizeVLQ(compressTxOutAmount(amount)) +
...
func putCompressedTxOut(target []byte, amount uint64, pkScript []byte) int {
	offset := serialization.PutVLQ(target, compressTxOutAmount(amount))
...
func encodeFlags(isCoinBase bool, hasExpiry bool, txType types.TxType, fullySpent bool) byte {
	b := uint8(txType)
	b <<= txTypeShift
...
// data types.
func decodeFlags(b byte) (bool, bool, types.TxType, bool) {
	isCoinBase := b&0x01 != 0
	hasExpiry := b&(1<<1) != 0
	fullySpent := b&(1<<4) != 0
	txType := types.TxType((b & txTypeBitmask) >> txTypeShift)
