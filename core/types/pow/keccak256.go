...
func (this *Keccak256) GetPowResult() json.PowResult {
...
func (this *Keccak256) Verify(headerData []byte, blockHash hash.Hash, targetDiffBits uint32) error {
	...
	if target.Cmp(this.params.Keccak256PowLimit) > 0 {
		str := fmt.Sprintf("block target difficulty of %064x is "+
			"higher than max of %064x", target, this.params.Keccak256PowLimit)
		return errors.New(str)
	}
	h := hash.HashKeccak256(headerData)
	hashNum := HashToBig(&h)
...
func (this *Keccak256) GetNextDiffBig(weightedSumDiv *big.Int, oldDiffBig *big.Int, currentPowPercent *big.Int) *big.Int {
...
func (this *Keccak256) PowPercent() *big.Int {
	targetPercent := big.NewInt(int64(this.params.GetPercentByHeight(this.mainHeight).Keccak256Percent))
	targetPercent.Lsh(targetPercent, 32)
	return targetPercent
}

func (this *Keccak256) GetSafeDiff(cur_reduce_diff uint64) *big.Int {
	limitBits := this.params.Keccak256PowLimitBits
	...
	if newTarget.Cmp(this.params.Keccak256PowLimit) > 0 {
		newTarget.Set(this.params.Keccak256PowLimit)
	}
	return newTarget
}
...
func (this *Keccak256) CompareDiff(newTarget *big.Int, target *big.Int) bool {
	return newTarget.Cmp(target) <= 0
}

// pow proof data
func (this *Keccak256) Bytes() PowBytes {
	r := make(PowBytes, 0)
	//write nonce 4 bytes
	n := make([]byte, 4)
...
// pow proof data
func (this *Keccak256) BlockData() PowBytes {
	l := len(this.Bytes())
	return PowBytes(this.Bytes()[:l-PROOFDATA_LENGTH])
}

//check pow is available
func (this *Keccak256) CheckAvailable() bool {
	return this.params.GetPercentByHeight(this.mainHeight).Keccak256Percent > 0
}
