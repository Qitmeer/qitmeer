...
type IPow interface {
    ...
    Verify(headerWithoutProofData []byte,targetDiff uint32,powConfig *PowConfig) error
    //set header nonce
...
//block hash
func (this *Pow) GetBlockHash (data []byte) hash.Hash {
    return hash.DoubleHashH(this.GetBlockData(data))
}
