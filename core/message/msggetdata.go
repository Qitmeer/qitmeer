...
// It is used to request data such as blocks and transactions from another peer.
// It should be used in response to the inv (MsgInv) message to request the
// actual data referenced by each inventory vector the receiving peer doesn't
// already have.  Each message is limited to a maximum number of inventory
// vectors, which is currently 50,000.  As a result, multiple messages must
// be used to request larger amounts of data.
//
...
// NewMsgGetData returns a new getdata message that conforms to the
...
// NewMsgGetDataSizeHint returns a new getdata message that conforms to
