...
var partial uint32
var partial_len uint
var round_shift uint
var S []uint32
...
func SMIX(x0, x1, x2, x3 *uint32) {
	...
	tmp = mixtab3[*x3&0xFF]
	c3 ^= tmp
	*x0 = ((c0 ^ r0) & (0xFF000000)) | ((c1 ^ r1) & (0x00FF0000)) | ((c2 ^ r2) & (0x0000FF00)) | ((c3 ^ r3) & (0x000000FF))
...
func fugue_init_c_code(z_len uint, iv []uint32, iv_len uint) {
	S = make([]uint32, 36)

	for u := 0; uint(u) < z_len; u++ {
		S[u] = 0
	}
	for u := 0; uint(u) < iv_len; u++ {
		S[z_len+uint(u)] = iv[u]
	}
...
func fugue4_core_c_code(data []uint8, length uint) {
	var p uint32
	var plen, rshift uint
	bit_count += uint64(length) << 3
	p = partial
	plen = partial_len
	if plen < 4 {
		count := 4 - plen
	...
	L0:
		...
		p = binary.BigEndian.Uint32(data)
		data = data[4:]
		length -= 4
...
func CLOSE_ENTRY(S1 []uint32, ub, n uint) {
	...
	var plen, rms uint
	plen = partial_len
	binary.BigEndian.PutUint64(buf[4:], bit_count+uint64(n))
	if plen == 0 && n == 0 {
		plen = 4
	} else if plen < 4 || n != 0 {
		...
		buf[plen] = uint8(ub & (^(0xFF >> n)))
		for u := plen + 1; u < 4; u++ {
			buf[u] = 0
		}
	}
	fugue4_core_c_code(buf[plen:], 16-plen)
	rms = round_shift * (12)
	for i := 0; uint(i) < rms; i++ {
		S1[i] = S[36-rms+uint(i)]
	}
	for i := 0; uint(i) < 36-rms; i++ {
		S1[rms+uint(i)] = S[i]
	}
}
...
func fugue4_close_c_code(ub, n uint, dst []uint8) {
	S1 := make([]uint32, 36)
	var i int
	...
	for i = 0; i < 32; i++ {
		ROR(S1, 3)
		CMIX36(&S1[0], &S1[1], &S1[2], &S1[4], &S1[5], &S1[6], &S1[18], &S1[19], &S1[20])
		SMIX(&S1[0], &S1[1], &S1[2], &S1[3])
	}
	for i = 0; i < 13; i++ {
		S1[4] ^= S1[0]
		S1[9] ^= S1[0]
	...
	binary.BigEndian.PutUint32(dst[0:4], S1[1])
	binary.BigEndian.PutUint32(dst[4:8], S1[2])
	binary.BigEndian.PutUint32(dst[8:12], S1[3])
	binary.BigEndian.PutUint32(dst[12:16], S1[4])
	binary.BigEndian.PutUint32(dst[16:20], S1[9])
	binary.BigEndian.PutUint32(dst[20:24], S1[10])
	binary.BigEndian.PutUint32(dst[24:28], S1[11])
	binary.BigEndian.PutUint32(dst[28:32], S1[12])
	binary.BigEndian.PutUint32(dst[32:36], S1[18])
	binary.BigEndian.PutUint32(dst[36:40], S1[19])
	binary.BigEndian.PutUint32(dst[40:44], S1[20])
	binary.BigEndian.PutUint32(dst[44:48], S1[21])
	binary.BigEndian.PutUint32(dst[48:52], S1[27])
	binary.BigEndian.PutUint32(dst[52:56], S1[28])
	binary.BigEndian.PutUint32(dst[56:60], S1[29])
