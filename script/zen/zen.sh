...
# ---------------------------
DEBUG=0
DEBUG_FILE=/tmp/zen_curl_debug
ERR_FILE=/tmp/zen_curl_error
...
function get_result_json_get(){

  set_host_port 

  local method=$1
  local params=$2
  local endpoint="http://$host:$port/$method/$params"

  local curl_result=$(curl -s -X GET -H 'Content-Type: application/json' $endpoint)
  local result=$(echo $curl_result|jq -r -c -M '.')
  if [ $DEBUG -gt 0 ]; then
    local curl_cmd="curl -s -X GET -H 'Content-Type: application/json' $endpoint"
    echo "$curl_cmd" > $DEBUG_FILE
    echo "$curl_result" >> $DEBUG_FILE
  fi
  if [ "$result" == "null" ];then
...
function get_tx_by_hash_get(){
  local tx_hash=$1
  local method="blockchain/transaction"
  local params="?hash=$tx_hash"
  get_result_json_get "$method" "$params"
}
...
function get_block_by_hash_post(){
  local block_hash=$1
  local data='{"jsonrpc":"2.0","method":"eth_getBlockByHash","params":["'$block_hash'",true],"id":1}'
  get_result "$data"
}

function get_block_by_hash_get(){

# curl http://127.0.0.1:5555/blockchain/block?hash=xxx
#
  local hash=$1
  local method="blockchain/block"
  local params="?hash=$hash"
  get_result_json_get "$method" "$params" 
}
...
function call_get_block() {
  ...
  if [ "$show" == "rlp" ]; then
    ...
    if ! [ "$blknum" == "" ]; then
       ...
       block_result=$(get_block_by_hash_get $blkhash)
    else
