...
# ---------------------------
DEBUG=0
DEBUG_FILE=/tmp/ss_curl_debug
ERR_FILE=/tmp/ss_curl_error
...
# https://github.com/paritytech/substrate/blob/master/core/rpc/src/chain/mod.rs
...
function get_block(){
  ...
  local data='{"jsonrpc":"2.0","method":"chain_getBlockHash","params":['$height'],"id":1}'
  get_result "$data"
}
...
function get_block_by_hash(){
  ...
  local data='{"jsonrpc":"2.0","method":"chain_getBlock","params":["'$block_hash'"],"id":1}'
  get_result "$data"
}
...
function call_get_block() {
  ...
  if [ "$show" == "rlp" ]; then
    ...
    if ! [ "$blknum" == "" ]; then
       ...
       block_result=$(get_block_by_hash $block_result)
    elif ! [ "$blkhash" == "" ]; then
