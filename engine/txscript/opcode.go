...
var opcodeArray = [256]Opcode{
	...
	OP_CHECK_UTXO_LOCK: {OP_CHECK_UTXO_LOCK, "OP_CHECK_UTXO_LOCK", 1, opcodeCheckSpentLockTime},
...
func opcodeCheckLockTimeVerify(op *ParsedOpcode, vm *Engine) error {
	...
	// setting its sequence to the maximum value (types.MaxTxInSequenceNum).  This
...
func opcodeCheckSequenceVerify(op *ParsedOpcode, vm *Engine) error {
	...
	lockTimeMask := int64(types.SequenceLockTimeIsSeconds |
		types.SequenceLockTimeMask)
	return verifyLockTime(txSequence&lockTimeMask,
		types.SequenceLockTimeIsSeconds,
...
func calcHash(buf []byte, hasher hhash.Hash) []byte {
	hasher.Write(buf)
...
func opcodeHash256(op *ParsedOpcode, vm *Engine) error {
	...
	vm.dstack.PushByteArray(hash.HashB(hash.HashB(buf)))
	return nil
}
...
func opcodeCheckSpentLockTime(op *ParsedOpcode, vm *Engine) error {
	err := opcodeVerify(op, vm)
	if err != nil {
		return err
	}
	so := vm.scripts[1][6].data
	lockTime, err := makeScriptNum(so, vm.dstack.verifyMinimalData, 5)
	if err != nil {
		return err
	}
