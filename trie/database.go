...
type Database struct {
	diskdb statedb.Database // Persistent storage for matured trie nodes

	nodes  map[hash.Hash]*cachedNode // Data and references relationships of a node
...
func (n *cachedNode) childs() []hash.Hash {
...
func gatherChildren(n node, children *[]hash.Hash) {
	...
	case hashNode:
		*children = append(*children, hash.MustBytesToHash(n))
...
func NewDatabase(diskdb statedb.Database) *Database {
	return &Database{
		diskdb:    diskdb,
		nodes:     map[hash.Hash]*cachedNode{{}: {}},
...
func (db *Database) insertPreimage(h hash.Hash, preimage []byte) {
	...
	db.preimages[h] = util.CopyBytes(preimage)
	db.preimagesSize += common.StorageSize(hash.HashSize + len(preimage))
}

// node retrieves a cached trie node from memory, or returns nil if none can be
// found in the memory cache.
func (db *Database) node(h hash.Hash, cachegen uint16) node {
...
func (db *Database) Node(h hash.Hash) ([]byte, error) {
	// Retrieve the node from cache if available
	db.lock.RLock()
	node := db.nodes[h]
	db.lock.RUnlock()
	...
	// Content unavailable in memory, attempt to retrieve from disk
	return db.diskdb.Get(h[:])
}

// preimage retrieves a cached trie node pre-image from memory. If it cannot be
// found cached, the method queries the persistent database for the content.
func (db *Database) preimage(h hash.Hash) ([]byte, error) {
	// Retrieve the node from cache if available
	db.lock.RLock()
	preimage := db.preimages[h]
	db.lock.RUnlock()
	...
	// Content unavailable in memory, attempt to retrieve from disk
	return db.diskdb.Get(db.secureKey(h[:]))
}
...
func (db *Database) Nodes() []hash.Hash {
	...
	var hashes = make([]hash.Hash, 0, len(db.nodes))
...
func (db *Database) reference(child hash.Hash, parent hash.Hash) {
	...
	if db.nodes[parent].children == nil {
		db.nodes[parent].children = make(map[hash.Hash]uint16)
	} else if _, ok = db.nodes[parent].children[child]; ok && parent != (hash.Hash{}) {
		return
	}
	node.parents++
...
func (db *Database) dereference(child hash.Hash, parent hash.Hash) {
	...
	if node.parents == 0 {
		...
		case db.oldest:
			...
			db.nodes[node.flushNext].flushPrev = hash.Hash{}
		case db.newest:
			db.newest = node.flushPrev
			db.nodes[node.flushPrev].flushNext = hash.Hash{}
		default:
		...
		for _, h := range node.childs() {
			db.dereference(h, child)
		}
		delete(db.nodes, child)
		db.nodesSize -= common.StorageSize(hash.HashSize + int(node.size))
	}
}
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	size := db.nodesSize + common.StorageSize((len(db.nodes)-1)*2*hash.HashSize)

	// If the preimage cache got large enough, push to disk. If it's still small
	// leave for later to deduplicate writes.
	flushPreimages := db.preimagesSize > 4*1024*1024
	if flushPreimages {
		for h, preimage := range db.preimages {
			if err := batch.Put(db.secureKey(h[:]), preimage); err != nil {
				log.Error("Failed to commit preimage from trie database", "err", err)
				db.lock.RUnlock()
				return err
			}
			if batch.ValueSize() > statedb.IdealBatchSize {
	...
	for size > limit && oldest != (hash.Hash{}) {
		...
		size -= common.StorageSize(3*hash.HashSize + int(node.size))
	...
	if db.oldest != (hash.Hash{}) {
		db.nodes[db.oldest].flushPrev = hash.Hash{}
	}
...
func (db *Database) commit(hash hash.Hash, batch statedb.Batch) error {
...
func (db *Database) Size() (common.StorageSize, common.StorageSize) {
	...
	// the total memory consumption, the maintenance metadata is also needed to be
	// counted. For every useful node, we track 2 extra hashes as the flushlist.
	var flushlistSize = common.StorageSize((len(db.nodes) - 1) * 2 * hash.HashSize)
...
func (db *Database) verifyIntegrity() {
	...
	for child := range db.nodes[hash.Hash{}].children {
		db.accumulate(child, reachable)
	}
	// Find any unreachable but cached nodes
	unreachable := []string{}
	for h, node := range db.nodes {
		if _, ok := reachable[h]; !ok {
			unreachable = append(unreachable, fmt.Sprintf("%x: {Node: %v, Parents: %d, Prev: %x, Next: %x}",
				h, node.node, node.parents, node.flushPrev, node.flushNext))
		}
	}
